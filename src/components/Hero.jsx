...
export default function Hero() {
  ...
  // The hero recedes into depth and dissolves as it scrolls away, while "Sobre
  // mí" materialises from depth right behind it — the two overlap, so the
  // hand-off is a genuine cross-fade.
  //
  // Deliberately unpinned. Pinning the hero here empties the viewport: while a
  // pin holds, nothing else can scroll in behind it, so fading the hero to zero
  // over that stretch leaves ~580px of blank screen (measured) before the next
  // section arrives — the same class of gap fixed elsewhere on the page.
  //
  // This also supersedes the plain translate/fade the scroll engine used to
  // apply here, so the engine no longer touches the hero.
  useEffect(() => {
    const hero = heroRef.current;
    const inner = innerRef.current;
    const about = document.getElementById('sobre-mi');
    if (!hero || !inner || !about) return undefined;

    const mm = gsap.matchMedia();
    mm.add(
      { reduce: '(prefers-reduced-motion: reduce)', motion: '(prefers-reduced-motion: no-preference)' },
      (context) => {
        // Reduced motion: a plain fade on the way out, no pin, no scaling.
        if (context.conditions.reduce) {
          gsap.fromTo(
            inner,
            { opacity: 1 },
            { opacity: 0, ease: 'none', scrollTrigger: { trigger: hero, start: 'top top', end: 'bottom top', scrub: true } }
          );
          return;
        }
        gsap.fromTo(
          ...
          { scale: 1, opacity: 1 },
          {
            ...
            scrollTrigger: {
              ...
              // Above every trigger below it, so refreshes still measure top-down.
              refreshPriority: 3
            }
          }
        );
  ...
  return (
    <section
      ...
      style={{
        ...
        padding: 'clamp(110px,15vh,180px) clamp(20px,5vw,60px) 0', color: '#F4F1EC', perspective: 1200
      }}
    >
      <div
        ref={innerRef}
        className="hero-grid"
        style={{
          flex: 1, minWidth: 0, display: 'grid', alignItems: 'center', gap: 'clamp(20px,3vw,50px)',
          transformOrigin: '50% 45%', willChange: 'transform, opacity'
        }}
      >
